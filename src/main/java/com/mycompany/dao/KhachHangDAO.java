...
public class KhachHangDAO extends QuanLyXeMayDAO<KhachHang, String> {
    ...
    String SELECT_BY_MATK = "select * from KhachHang where maTaiKhoan=?";
    ...
    public KhachHang slectByMaTK(String maTK) {
        List<KhachHang> list = selectBySQL(SELECT_BY_MATK, maTK);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
